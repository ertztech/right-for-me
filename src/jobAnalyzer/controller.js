...
function analyze(jobText, backgroundText) {
  ...
  return {
    ...
    stretch,
    nextMove,
    summary: `This role appears to emphasize ${
      jobSignals.slice(0, 6).join(", ") || "a mix of responsibilities that need closer review"
    }. The first-pass read is ${fit >= 60 ? "promising" : "still developing"}, with ${
      stretch >= 60 ? "meaningful stretch areas" : "manageable stretch areas"
    }.`,
    evidence,
    stretchAreas,
    strategy:
